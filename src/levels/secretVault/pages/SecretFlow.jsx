...
const SecretFlow = () => {
  return (
    <div className="secret-flow">
        ...
        <p>To identify the request sender, the authenticated user’s user ID is included in the request, preventing unauthorized users without the same user ID from making requests. In systems using cookie-based session ID storage, threats like Cross-Site Request Forgery (CSRF) and Cross-Site Scripting (XSS) attacks arise.</p>
        <p>In a CSRF attack, an attacker tricks a user into unintended requests, exploiting their authenticated session cookie. In an XSS attack, malicious scripts injected into a trusted website may lead to the unauthorized access of a user’s account by stealing their session cookies.</p>
